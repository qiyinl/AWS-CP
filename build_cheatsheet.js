...
try {
    ...
    console.log(`Generated cheatsheet_static.html with ${questions.length} questions.`);
} catch (err) {
...
function generateHtml(questions) {
    ...
    for (const topic of sortedTopics) {
        ...
        `;
        rows += grouped[topic].map(q => {
            const answers = q.answerIndices.map(i => q.options[i]).join('<br><br>');
            return `
                <tr>
